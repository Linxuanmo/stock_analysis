...
PERIOD_DEFAULT_DIRS = {
    '1': {  # 日线周期的配置
        'label': '日线',                     # 显示给用户的名称
        'path': r"D:\desk_file\Me\school\大一下\股票\stock_analysis\data\raw\daily\2026_5_29__688__50__total"  # 默认文件夹路径（原始字符串）
    },
    '2': {  # 月线周期的配置（若暂无月线，路径可暂时留空，程序会提示输入）
        'label': '月线',
        'path': r"D:\desk_file\Me\school\大一下\股票\stock_analysis\data\raw\monthly\2026_06_07__688__30__monthly"
    },
    '3': {  # 自定义周期（用户手动输入路径）
        'label': '自定义周期',
        'path': ''                           # 空字符串，意味着必须由用户输入
    }
}
